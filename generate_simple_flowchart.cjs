...
const drawPageHeader = (subtitle) => {
  ...
  doc.text(subtitle, pw - mr, 14.8, { align: 'right' });
  ...
  doc.text('IDS Pulse - Simple App Workflow & Structures Overview', pw / 2, 289, { align: 'center' });
...
const h4 = drawInfoCard(ml + 63, y, 56, 26, 'Floor Activities', [
  ...
  'Logs hours worked and expenses.',
], C.teal);
...
const stepW = 41;
const hw1 = drawWorkflowStep(ml, y, stepW, 36, 1, 'Capture & Mark', 'Rep captures up to three photos (wide, medium, close-up) and draws red arrows or annotations on the screen to highlight the defect.', C.midBlue);
...
const hw4 = drawWorkflowStep(ml + stepW * 3 + 15, y, 37, 36, '4', 'AI Check & Send', 'App checks for recent duplicate logs. If clean, representative previews the HTML email and clicks submit to notify the supplier.', C.purple);
...
// ════════════════════════════════════════════════════════════════════════════
// PAGE 2 — WEB DASHBOARD & CUSTOMER PORTAL OVERVIEW
// ════════════════════════════════════════════════════════════════════════════
newPage('Page 2 - Web Dashboard & Customer Portal');

y = 25;
y = drawSectionHeader(y, 4, 'Web Corporate Dashboard (For Administrators)');

const h6 = drawInfoCard(ml, y, 86, 32, 'Operations Management', [
  'Incident defective feed: Real-time logs of defect activities on the floor.',
  'Visual Defect Heatmap: interactive part diagram showing defect clusters.',
  'Shift walkthrough summaries: log of walkthrough checklists and daily tasks.',
  'Suppliers directory: active contact list of client representatives.',
], C.navy);

const h7 = drawInfoCard(ml + 92, y, 86, 32, 'Finance & Billing Control', [
  'Timesheets payroll panel: records of logged hours and representative mileage.',
...
y += Math.max(h6, h7) + 6;
y = drawSectionHeader(y, 5, 'Web Customer Portal (For Clients)');
...
// ════════════════════════════════════════════════════════════════════════════
// PAGE 3 — APPROVAL WORKFLOWS & DATAFLOWS
// ════════════════════════════════════════════════════════════════════════════
newPage('Page 3 - Simple Approval Workflows');

y = 25;
y = drawSectionHeader(y, 6, 'Overtime (Extra Hours) Approval Workflow');
...
y = drawSectionHeader(y, 7, 'Expense Claims Approval Workflow');
...
// ─── SAVE ───────────────────────────────────────────────────────────────────
const outputPath = path.join(__dirname, 'IDS_Pulse_Software_Flowchart_Simple.pdf');
