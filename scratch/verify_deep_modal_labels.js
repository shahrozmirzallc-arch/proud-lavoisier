...
async function run() {
  console.log('[Deep Modal Label Verification] Capturing updated modal screenshot...');
  ...
  const imgModal = path.join(ARTIFACTS_DIR, '09_deep_audited_onboarding_modal.png');
  await page.screenshot({ path: imgModal });
  console.log(`Saved screenshot: ${imgModal}`);

  await browser.close();
  console.log('[Deep Modal Label Verification] Complete!');
}

run().catch(err => {
  console.error('[Deep Modal Label Verification Error]:', err);
  process.exit(1);
});
