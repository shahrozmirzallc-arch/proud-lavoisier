...
async function run() {
  console.log('[Inline Title Dropdown Verification] Checking updated options...');
  ...
  // Click "+ Create New IDS Field Rep" to open inline creation card
  await page.evaluate(() => {
    const btns = Array.from(document.querySelectorAll('button'));
  ...
  const imgCard = path.join(ARTIFACTS_DIR, '10_inline_ids_field_rep_card.png');
  await page.screenshot({ path: imgCard });
  console.log(`Saved screenshot: ${imgCard}`);

  await browser.close();
  console.log('[Inline Title Dropdown Verification] Complete!');
}

run().catch(err => {
  console.error('[Inline Title Dropdown Verification Error]:', err);
  process.exit(1);
});
