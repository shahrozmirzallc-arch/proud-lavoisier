...
async function run() {
  console.log('[Removed Title Dropdown Verification] Capturing simplified inline creation card screenshot...');
  ...
  // Click "+ Create New IDS Field Rep"
  await page.evaluate(() => {
    const btns = Array.from(document.querySelectorAll('button'));
    const target = btns.find(b => b.innerText.includes('Create New IDS Field Rep'));
    if (target) target.click();
  });
  await new Promise(r => setTimeout(r, 1500));

  const imgSimplified = path.join(ARTIFACTS_DIR, '11_simplified_inline_rep_card.png');
  await page.screenshot({ path: imgSimplified });
  console.log(`Saved screenshot: ${imgSimplified}`);

  await browser.close();
  console.log('[Removed Title Dropdown Verification] Complete!');
}

run().catch(err => {
  console.error('[Removed Title Dropdown Verification Error]:', err);
  process.exit(1);
});
